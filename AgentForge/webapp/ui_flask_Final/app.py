...
from adaptaters.simple_storage import SimpleProjectStorage
...
try:
    ...
    from graph.engine import Node, GraphConfig, GraphRunner
...
class UIAwareOrchestrator:
    ...
    def run_pipeline_with_ui(self, prompt: str, session_id: str, demo_mode: bool = False):
        ...
        # Emit demo mode status if enabled
        if demo_mode:
            socketio.emit('info', {'message': '🎭 Mode démo activé - Traitement accéléré', 'session_id': session_id})
...
def system_status_alt():
    """Alternative route for system status (frontend compatibility)"""
...
def handle_start_pipeline(data):
    """Handle pipeline start request with optional demo mode"""
    prompt = data.get('prompt', '')
    session_id = data.get('session_id', f'session_{int(time.time())}')
    demo_mode = data.get('demo_mode', False)  # New demo mode parameter
    ...
    if demo_mode:
        emit('info', {'message': '🎭 Mode démo activé - Traitement accéléré'})
    
    # Run pipeline in background thread
    def run_pipeline():
        ui_orchestrator.run_pipeline_with_ui(prompt, session_id, demo_mode=demo_mode)
