            ...
            try:
                ...
                if result and result.get('code'):
                    ...
                    if isinstance(code_content, list):
                        # If LLM returned a list, join the lines
                        ...
                        elif len(code_content) == 1:
                            # If dict has one key, use its value
                            code_content = list(code_content.values())[0]
                            if isinstance(code_content, list):
                                code_content = '\n'.join(str(line) for line in code_content)
                        else:
                    ...
                    # Skip if content is too short (probably not real code)
                    if len(code_content.strip()) < 10:
                        actions.append(f"❌ Generated content too short for {file_path}")
                        continue
                    ...
                    # If JSON parsing completely failed, try raw response
                    print(f"   🔧 JSON failed for {file_path}, trying raw response...")
                    raw_response = self.llm_client.get_raw_response(
                        system_prompt="You are a senior software engineer. Write complete, working code.",
                        user_prompt=f"Write the complete code for {file_path} - {file_purpose}. No explanations, just code."
                    )
                    ...
                    if raw_response and len(raw_response.strip()) > 50:
                        ...
                        file_full_path.write_text(code_content)
                        files_written += 1
                        actions.append(f"✅ LLM generated {file_path} from raw response ({len(code_content)} chars)")
                    else:
                        actions.append(f"❌ Failed to generate {file_path} - no usable response")
                else:
                    actions.append(f"❌ Failed to generate {file_path} - empty code field")
        ...
        try:
            ...
            if result:
                ...
                if result.get('schema_sql'):
                    schema_content = result['schema_sql']
                    # Handle both string and dict responses
                    if isinstance(schema_content, dict):
                        # Convert dict to readable SQL/NoSQL schema
                        if 'collections' in schema_content:
                            # MongoDB-style schema
                            schema_text = "// MongoDB Schema\n"
                            for collection in schema_content.get('collections', []):
                                schema_text += f"db.{collection.get('name', 'collection')}.createIndex(...);\n"
                        else:
                            # Convert dict to JSON schema
                ...
                if result.get('config'):
                    ...
                    config_file.write_text(str(config_content))
                    actions.append("✅ LLM generated database config")
                
                # Write init script
                if result.get('init_script'):
                    init_content = result['init_script']
                    if isinstance(init_content, dict):
                        init_content = json.dumps(init_content, indent=2)
                    init_file = self.project_dir / "init_database.py"
                    init_file.write_text(str(init_content))
            ...
            if result:
                ...
                if result.get('dockerfile'):
                    ...
                    if isinstance(dockerfile_content, dict):
                        dockerfile_content = str(dockerfile_content)
                ...
                if result.get('docker_compose'):
                    ...
                    if isinstance(compose_content, dict):
                        compose_content = str(compose_content)
                ...
                if result.get('kubernetes'):
                    k8s_content = result['kubernetes']
                    if isinstance(k8s_content, dict):
                        k8s_content = json.dumps(k8s_content, indent=2)
                    k8s_dir = self.project_dir / "k8s"
                    k8s_dir.mkdir(exist_ok=True)
                    (k8s_dir / "deployment.yaml").write_text(str(k8s_content))
                    actions.append("✅ LLM generated Kubernetes manifests")
                
                # Write CI/CD
                if result.get('ci_cd'):
                    ci_cd_content = result['ci_cd']
                    if isinstance(ci_cd_content, dict):
                        ci_cd_content = json.dumps(ci_cd_content, indent=2)
                    github_dir = self.project_dir / ".github" / "workflows"
                    github_dir.mkdir(parents=True, exist_ok=True)
                    (github_dir / "ci.yml").write_text(str(ci_cd_content))
