...
class MultiPerspectiveTechAgent(LLMBackedMixin):
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        track_llm_call("🎭 MultiPerspectiveTechAgent", "organic team decision")
...
Return JSON with your organic team decision:
{
  ...
  "team_discussion": "summary of the multi-perspective team meeting"
        ...
        # Build context with experience guidance (not tech forcing)
        ...
        result = self.llm_json(system_prompt, user_prompt, fallback_team)
        ...
        if result.get('team_discussion'):
            print(f"   💬 Team process: {result['team_discussion'][:100]}...")
...
class CodeGenAgent(LLMBackedMixin):
    ...
    def can_run(self, state: Dict[str, Any]) -> bool:
        return 'generated_code' not in state and 'architecture' in state and 'tech_stack' in state
...
class ValidateAgent(LLMBackedMixin):
    ...
    def can_run(self, state: Dict[str, Any]) -> bool:
        return 'validation' not in state and 'generated_code' in state
...
class EvaluationAgent(LLMBackedMixin):
    ...
    def can_run(self, state: Dict[str, Any]) -> bool:
        return 'evaluation' not in state and 'validation' in state
